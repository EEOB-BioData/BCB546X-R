...
> you enter this information, a new tab opens with a new R Markdown file. 
> Notice that the file contains three types of content: An (optional) YAML header 
> surrounded by ---s; R code chunks surrounded by ```s; and text mixed with simple 
> text formatting. You can use the “Knit” button in the RStudio IDE to render the 
...
> the right. To run the current line, you can 1. click on the `Run` button above the 
> editor panel, or 2. select "Run Lines" from the "Code" menu, or 3. hit Ctrl-Enter 
> in Windows or Linux or Command-Enter on OS X. 
{: .callout}
