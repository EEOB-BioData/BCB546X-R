...
> ## Subsetting rows with `which` and `subset`
...
Here we’ll merge two datasets to explore recombination 
rates around a degenerate sequence motif that occurs in repeats.  The first dataset (motif_recombrates.txt) contains 
